...
function getProposal(num){
	...
	msg += `<b>🗳️프로포절</b>\n\n`
	msg += `#${num} ${title}\n\n`
	msg += `📌<a href='https://www.mintscan.io/juno/proposals/${num}'>https://www.mintscan.io/juno/proposals/${num}</a>\n\n`
	msg += `🔍다른 프로포절 검색은 [/proposal 숫자]\n\n`
	msg += `<b>프로밸리와 $JUNO 스테이킹 하세요❤</b>\n\n`
	msg += `<b>🏆검증인 순위: #${prvRank}</b>\n\n`
	msg += `<b>🔖수수료: ${prvRate}%</b>\n\n`
	msg += `<b>🤝위임량: ${numberWithCommas(prvTokens)}</b>\n\n`
	msg += `ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ\n`
...
function getProposalFromServer(num){ //write Proposal json
	let json = fetch(process.env.JUNO_API_URL+"/gov/proposal/"+num).json()
...
function getPrice(){
	let json = fetch('https://api.coingecko.com/api/v3/simple/price?ids=juno-network&vs_currencies=usd,krw').json()
	json = JSON.stringify(json).replace("juno-network","juno")
	json = JSON.parse(json)
	return [json.juno.usd,json.juno.krw]
}
